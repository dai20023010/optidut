...
mon_hocs = list(MonHoc.objects.all().order_by('ma_mon')[:50])
lop_hocs = list(LopHoc.objects.all().order_by('ten_lop')[:50])
giang_viens = list(NguoiDung.objects.filter(vai_tro='giang_vien', is_active=True).order_by('ma_so')[:50])
phongs = list(PhongHoc.objects.filter(trang_thai='trong').order_by('ma_phong')[:50])
